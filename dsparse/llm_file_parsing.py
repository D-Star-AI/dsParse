...
def get_llm_provider(file_parsing_model: str) -> str:
    if file_parsing_model == "gpt-4o-mini":
        return "openai"
    elif file_parsing_model == "gemini-1.5-flash-001":
        return "gemini"
    else:
        raise ValueError("Invalid file parsing model. Must be either 'gpt-4o-mini' or 'gemini-1.5-flash-001'")
    

def parse_file_llm(filepath: str, file_or_url: str, file_parsing_model: str):
    # Make sure the file is one of [.pdf, .ppt, .pptx, .png, or .jpg]
    if not filepath.endswith(('.pdf', '.ppt', '.pptx', '.png', '.jpg')):
        raise ValueError("File must be a PDF, PPT, PNG, or JPG file")
    
    # Get the llm provider from the file parsing model
    llm_provider = get_llm_provider(file_parsing_model)
    
    if llm_provider == "gemini":
        document_text = make_llm_call_gemini(filepath, file_parsing_model)
    elif llm_provider == "openai":
        document_text = make_llm_call_openai(filepath, file_parsing_model)
